...
<style>
:root {
  --bg:#F2F4F8; --surface:#FFFFFF; --border:#E5E7EB;
  --blue:#1B4FBB; --blue-l:#E8EEFA; --blue-mid:#2563EB;
  --red:#DC2626; --red-l:#FEF2F2; --amber:#D97706; --amb-l:#FFFBEB;
  --green:#16A34A; --grn-l:#F0FDF4;
  --ink:#111827; --sub:#374151; --muted:#6B7280;
...
/* 앱바 */
.ax-topbar{background:var(--surface);border-bottom:1px solid var(--border);
  display:flex;align-items:center;padding:0 24px;height:54px;
  position:sticky;top:0;z-index:200;}
.ax-logo{display:flex;align-items:center;gap:10px;padding-right:28px;
  border-right:1px solid var(--border);margin-right:24px;white-space:nowrap;}
.ax-logo-icon{width:32px;height:32px;border-radius:6px;background:var(--blue);
  display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff;
  font-weight:800;font-size:.75rem;letter-spacing:-.02em;}
.ax-logo-text{font-size:.82rem;line-height:1.3;}
.ax-logo-text .l1{font-weight:700;color:var(--ink);display:block;}
.ax-logo-text .l2{font-size:.71rem;color:var(--muted);display:block;}
.ax-nav{display:flex;gap:0;flex:1;height:100%;}
.ax-nav-item{height:100%;display:flex;align-items:center;padding:0 16px;
  font-size:.83rem;color:var(--muted);cursor:pointer;white-space:nowrap;
...
.ax-badge{background:var(--blue-l);color:var(--blue);
  font-size:.71rem;font-weight:600;padding:2px 8px;border-radius:20px;}
/* 메인 */
.ax-main{padding:20px 24px;max-width:1440px;margin:0 auto;}
/* 페이지 헤더 */
.ax-page-header{display:flex;align-items:flex-start;
  justify-content:space-between;margin-bottom:16px;}
.ax-page-title{font-size:1.25rem;font-weight:700;color:var(--ink);margin:0 0 2px;}
.ax-page-meta{font-size:.77rem;color:var(--muted);margin:0;}
/* KPI */
.ax-kpi-row{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:16px;}
.ax-kpi{background:var(--surface);border:1px solid var(--border);
  border-radius:var(--radius);padding:15px 17px;box-shadow:var(--shadow);
  display:flex;align-items:flex-start;justify-content:space-between;}
.ax-kpi .kpi-label{font-size:.72rem;color:var(--muted);font-weight:500;margin-bottom:5px;}
.ax-kpi .kpi-val{font-size:1.8rem;font-weight:700;line-height:1;
  margin-bottom:3px;font-family:var(--mono);}
.ax-kpi .kpi-sub{font-size:.7rem;color:var(--muted);}
.ax-kpi .kpi-icon{width:36px;height:36px;border-radius:8px;
...
/* 섹션 */
.ax-section{background:var(--surface);border:1px solid var(--border);
  border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden;margin-bottom:12px;}
.ax-section-head{display:flex;align-items:center;justify-content:space-between;
  padding:11px 16px;border-bottom:1px solid var(--border);}
.ax-section-head h3{font-size:.86rem;font-weight:700;color:var(--ink);margin:0;
  display:flex;align-items:center;gap:6px;}
.sh-badge{background:var(--blue-l);color:var(--blue);
  font-size:.68rem;font-weight:600;padding:1px 7px;border-radius:20px;}
.ax-section-body{padding:14px 16px;}
.ax-section-link{font-size:.75rem;color:var(--blue);cursor:pointer;}
/* 순찰 카드 */
.ax-patrol-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;}
.ax-patrol-card{border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;}
.ax-patrol-card .pc-top{padding:10px 13px 8px;border-bottom:1px solid var(--border);
  display:flex;align-items:center;gap:8px;}
.ax-patrol-card .pc-rank{width:21px;height:21px;border-radius:50%;background:var(--blue);
  color:#fff;font-family:var(--mono);font-size:.68rem;font-weight:700;
  display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.ax-patrol-card.urgent .pc-rank{background:var(--red);}
.ax-patrol-card .pc-id{font-family:var(--mono);font-size:.86rem;font-weight:700;color:var(--ink);}
.ax-patrol-card .pc-type{font-size:.69rem;background:#F3F4F6;color:var(--sub);
  border-radius:4px;padding:1px 6px;white-space:nowrap;}
.ax-patrol-card .pc-body{padding:9px 13px;}
.ax-patrol-card .pc-chips{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:7px;}
.ax-chip{font-size:.68rem;padding:2px 7px;border-radius:20px;
  background:#F3F4F6;color:var(--sub);border:1px solid var(--border);}
...
.ax-patrol-card .pc-action{font-size:.77rem;font-weight:600;color:var(--blue);}
.ax-patrol-card.urgent .pc-action{color:var(--red);}
/* 상황요약 */
.ax-summary-list{display:flex;flex-direction:column;gap:0;}
.ax-summary-row{display:flex;align-items:flex-start;gap:9px;
  padding:8px 0;border-bottom:1px solid var(--border);font-size:.8rem;}
.ax-summary-row:last-child{border-bottom:none;}
.ax-summary-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:4px;}
.dot-red{background:var(--red);} .dot-amber{background:var(--amber);}
.dot-blue{background:var(--blue);} .dot-gray{background:#D1D5DB;}
.ax-summary-row .sr-body{flex:1;line-height:1.45;color:var(--sub);}
.ax-summary-row .sr-body b{color:var(--ink);}
/* 분포 바 */
.ax-bar-row{display:flex;align-items:center;gap:10px;margin-bottom:6px;}
.ax-bar-label{font-size:.75rem;color:var(--sub);width:58px;flex-shrink:0;}
.ax-bar-track{flex:1;height:8px;background:#F3F4F6;border-radius:4px;overflow:hidden;}
.ax-bar-fill{height:100%;border-radius:4px;}
.ax-bar-val{font-size:.73rem;color:var(--muted);width:36px;
  text-align:right;flex-shrink:0;font-family:var(--mono);}
/* Power Few */
.ax-pf-row{display:flex;align-items:baseline;gap:8px;margin-bottom:7px;}
.ax-pf-pct{font-family:var(--mono);font-size:1.55rem;font-weight:800;color:var(--blue);}
.ax-pf-desc{font-size:.79rem;color:var(--muted);}
/* AI 판정 그리드 */
.ax-judge-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;}
.ax-judge-item{border:1px solid var(--border);border-radius:var(--radius);
  padding:9px 12px;background:var(--surface);}
.ax-judge-item .ji-label{font-size:.7rem;color:var(--muted);margin-bottom:3px;}
.ax-judge-item .ji-val{font-family:var(--mono);font-size:1.15rem;font-weight:700;color:var(--ink);}
.ax-judge-item .ji-sub{font-size:.68rem;color:var(--muted);}
...
/* 범례 */
.ax-legend{display:flex;flex-wrap:wrap;gap:10px;margin-top:7px;}
.ax-legend-item{display:flex;align-items:center;gap:5px;font-size:.72rem;color:var(--muted);}
.ax-legend-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;}
/* 탭 공용 */
.ax-tab-body{padding:0 24px 32px;max-width:1440px;margin:0 auto;}
.ax-panel{background:var(--surface);border:1px solid var(--border);
  border-radius:var(--radius);box-shadow:var(--shadow);padding:17px 19px;margin-bottom:13px;}
.ax-panel h4{font-size:.87rem;font-weight:700;color:var(--ink);margin:0 0 11px;}
/* 배지/노트 */
.rp-note{color:var(--muted);font-size:.81rem;line-height:1.6;}
.badge{display:inline-block;font-size:.73rem;padding:2px 9px;border-radius:5px;margin:2px 3px 2px 0;}
...
/* 브리핑 */
.headline-card{border-radius:var(--radius);padding:15px 17px;}
...
.brief-tag{font-family:var(--mono);font-size:.67rem;padding:2px 7px;
  border-radius:5px;color:#fff;display:inline-block;margin-bottom:7px;}
.note-row{display:flex;gap:9px;padding:8px 0;border-bottom:1px solid var(--border);font-size:.82rem;}
/* Streamlit 미세 조정 */
...
def render_home():
    ...
    # ── 2초 요약 배너 ─────────────────────────────────
    st.markdown(
        f"""<div class="rp-banner">
          <div class="rp-banner-hero">
            <span class="eyebrow">POWER FEW 분석 · 형곡지구대</span>
            <h1>RE:PEAT <span>112</span></h1>
            <p class="sub">반복되는 신고를 데이터로 찾아, 반복 출동을 문제해결로 전환합니다.</p>
          </div>
          <div class="rp-stat danger">
            <div class="st-label">재발가속 지점</div>
            <div class="st-val">{n_accel}</div>
            <div class="st-sub">간격이 짧아진 우선 대응</div>
          </div>
          <div class="rp-stat warn">
            <div class="st-label">Power Few 상위 5%</div>
            <div class="st-val">{conc_adj['top5']['share']*100:.0f}%</div>
            <div class="st-sub">{conc_adj['top5']['k_places']}곳 → 전체의 {conc_adj['top5']['share']*100:.0f}%</div>
          </div>
          <div class="rp-stat ok">
            <div class="st-label">맞춤형 순찰 지점</div>
            <div class="st-val">{n_patrol_total}</div>
            <div class="st-sub">확인대기 {n_pending}곳 별도</div>
          </div>
        </div>""",
        unsafe_allow_html=True,
    )
